...
# --- Streamlit entry (consolidated fix) ---
# - Unique keys for all widgets
# - Robust import path bootstrapping
# - Package+flat import fallback
# - Diagnostics panel
...
# 1) Ensure repo root and common subfolders are importable
paths = [str(ROOT), str(ROOT / "data_sources"), str(ROOT / "features"), str(ROOT / "model")]
for p in paths:
    if p not in sys.path:
        sys.path.insert(0, p)

# 2) Auto-create __init__.py so Python treats folders as packages
...
# 4) Import data fetchers (package style -> flat fallback)
try:
